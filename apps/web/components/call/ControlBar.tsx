...
function CtrlButton({
  ...
  return (
    <button
      ...
      title={label}
      className={clsx(
        "grid place-items-center w-12 h-12 rounded-full border transition-all",
        danger
...
export function ControlBar({ chatOpen, onToggleChat, unread, onLeave, extra }: Props) {
  ...
  return (
    <div className="glass rounded-full px-3 py-2.5 flex items-center gap-2 shadow-2xl">
      <CtrlButton
      ...
      <div className="w-px h-7 bg-panel-border mx-1" />
      ...
      <button
        ...
        title="Leave"
        className="grid place-items-center h-12 px-5 rounded-full bg-danger text-white font-medium hover:brightness-110 transition-all gap-2"
      >
